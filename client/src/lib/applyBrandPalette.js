...
/**
 * Pick solid button fill + text that meet WCAG AA (4.5:1) for any brand accent.
 */
export function buttonColorsFromAccent(accentRgbString) {
  ...
  // Darken accent until white text passes
  let bg = accent;
  ...
  // Bright accents: lighten + dark text
  bg = accent;
  for (let i = 0; i < 24; i += 1) {
...
export function applyBrandPalette(palette, mode = 'dark') {
  if (typeof document === 'undefined' || !palette) return null;
